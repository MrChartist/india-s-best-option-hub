/**
 * Jainam (XTS) broker integration. See registry.mjs for the shared contract.
 *
 * Jainam runs on Symphony Fintech's white-label "XTS" trading backend (see
 * jainamxtsAuth.mjs for the shared-backend note — compositedge/iifl/wisdom in
 * this same registry are almost certainly the same vendor stack under
 * different brand skins, based on openalgo's near-identical source for each).
 *
 * Auth model: server-side handshake, no TOTP — a direct appKey + secretKey
 * exchange against XTS's market-data login (jainamxtsAuth.mjs), cached
 ...
 * (jainamxtsInstruments.mjs), and price/OI are filled in via batched
 * /instruments/quotes calls (two xtsMessageCodes — 1502 for touchline,
 * 1510 for open interest — matching openalgo's production adapter). XTS
 * returns neither IV nor Greeks, so both are back-solved via Black-Scholes.
 */

import { xtsRequest, ensureSession } from "./jainamxtsAuth.mjs";
import {
  SYMBOL_CONFIG,
  assertKnownSymbol,
  getSegmentMaster,
...
async function resolveIndexInstrument(creds, symbol) {
  const tokenMap = await getIndexTokenMap(creds); // plain object, not a Map — see jainamxtsInstruments.mjs
...
export async function testConnection(creds = {}) {
  try {
    ...
    return { status: "success", message: `Jainam (XTS) market-data session authenticated${session.userID ? ` (user ${session.userID})` : ""}` };
  } catch (e) {
