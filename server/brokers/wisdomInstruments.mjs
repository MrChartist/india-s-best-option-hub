/**
 * Wisdom Capital (XTS) instrument-master + index-token resolution.
 *
 ...
 * verified against marketcalls/openalgo's production `broker/wisdom` adapter
 * (broker/wisdom/database/master_contract_db.py's `headers_fo` constant and
 * `process_compositedge_nfo_csv`), which downloads and parses this exact
 * endpoint successfully; the ContractExpiration column is whatever
 * pandas.to_datetime can parse (ISO-8601-shaped in practice) — handled
 * defensively below. OptionType 1=FUT, 3=CE, 4=PE per the same source.
 *
 ...
 * defensive fallback in case a given Wisdom deployment enforces it.
 */

import { getCachedOrFetch, ONE_DAY_MS } from "../lib/instrumentCache.mjs";
import { xtsRequest } from "./wisdomAuth.mjs";

// XTS numeric exchange-segment codes (openalgo's exchange_segment_map, both data.py and master_contract_db.py).
...
// Verified against openalgo's broker/wisdom/database/master_contract_db.py nse_index_map
// (process_index_data). SENSEX is not remapped there — BSE's raw index-list entry for it
// is already "SENSEX", so it passes through unchanged.
...
export function assertKnownSymbol(symbol) {
  if (!SYMBOL_CONFIG[symbol]) throw new Error(`Unknown symbol for Wisdom Capital: ${symbol}`);
}
...
async function downloadSegmentMaster(creds, masterSegment) {
  ...
  if (!result) throw new Error(`Wisdom Capital instrument master returned no data for ${masterSegment}`);
...
export async function getSegmentMaster(creds, masterSegment) {
  return getCachedOrFetch(`wisdom:master:${masterSegment}`, () => downloadSegmentMaster(creds, masterSegment), ONE_DAY_MS);
}

/** Distinct ascending ISO expiry dates for one app symbol's option contracts. */
export function listExpiries(rows, symbol) {
  const set = new Set();
  for (const r of rows) if (r.name === symbol) set.add(r.expiryIso);
...
async function downloadIndexList(creds) {
  const map = {};
  for (const segment of [SEGMENT.NSE_CM, SEGMENT.BSE_CM]) {
    let result;
    try {
    ...
    for (const entry of result?.indexList || []) {
      ...
      if (!symbol) continue;
      map[symbol] = { token, exchangeSegment: segment };
    }
  }
  return map;
}
...
export async function getIndexTokenMap(creds) {
  return getCachedOrFetch("wisdom:indexlist", () => downloadIndexList(creds), ONE_DAY_MS);
}
